...
```{r}
setwd("/Users/tobelemantares/OneDrive/Antares/Documents/Schools/Harvard University/04 Fourth Year/01 Gov 50/Projects/class-2020-09-17/raw_data/")
raw <- read_excel("class_enrollment.xlsx", skip = 3)
en_fall_2020 <- raw %>%
  select("Course Title", "Course Name", "UGrad", "Course Department") %>%
  filter(UGrad >= 10) %>%
  drop_na() %>%
  arrange(desc(UGrad))
en_fall_2020
